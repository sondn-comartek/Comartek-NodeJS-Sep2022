...
export class AuthService {
  ...
  async signup( { password, email , username }: RegiserInput): Promise<UserDocument | Record<string,unknown> >{
  ...
  async login( {email , password} : LoginInput): Promise<JwtTokens | Record<string,unknown>> {
    ...
    if (user?.status === UserStatus.INACTIVE) throw new UnauthorizedException('User inactive!')
    delete user.hash
