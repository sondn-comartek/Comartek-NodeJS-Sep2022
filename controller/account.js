...
const login=async (req,res,next)=>{
    ...
    const tokenShort=jwt.sign({email,time},process.env.JWT,{expiresIn:'0.25h'})
    const tokenLong=jwt.sign({email,time},process.env.JWT,{expiresIn:'30d'}) 
...
const forgotPass=async (req,res,next)=>{
    ...
    const {time}=rs
    const token=await jwt.sign({email,time},process.env.JWT,{expiresIn:'3d'})
