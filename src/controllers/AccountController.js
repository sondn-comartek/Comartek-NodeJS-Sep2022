...
export const register = async (req, res, next) => {
    try {
        ...
        if (!UserService.isMatchPassword(password, passwordConfirm)) {
...
export const login = async (req, res, next) => {
    try {
        ...
        bcrypt.compare(password, results.password, function (error, response) {
            console.log("vao day2", error, response)
            if (error) {
                console.log(error);
            } else {
                if (response == false) {
                    ...
                    const token = UserService.generateToken(results._id, email);
